...
function getFilteredData() {
    ...
    const range = getDateFilterRange(currentDateFilter);
    if (!range) return currentData;

    const filteredSummaries = currentData.summaries.filter(s => s.date >= range.start && s.date <= range.end);
    ...
    const perDevice = Object.entries(deviceGroups).map(([name, data]) => ({
        licenseAlias: name,
    ...
    const perDay = Object.entries(dayGroups).map(([date, data]) => ({
        date,
